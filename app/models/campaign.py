...
class VoiceBotCampaign(Base, AuditMixin):
    ...
    def __repr__(self):
        """
        Return a concise machine-readable representation of the campaign including its id and name.
        
        Returns:
            str: Representation in the format "<VoiceBotCampaign(id={id}, name={name})>" where `{id}` and `{name}` are the campaign's UUID and name.
        """
...
class CampaignContact(Base):
    ...
    def __repr__(self):
        """
        Provide a concise debug representation of the CampaignContact including its id and phone number.
        
        Returns:
            str: A string containing the contact's `id` and `phone`, suitable for debugging.
        """
