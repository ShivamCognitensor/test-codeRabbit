...
async def exotel_stream_url(request: Request):
    """Exotel AgentStream applet can fetch the WSS URL from this endpoint."""
    s = get_settings()
...
async def freeswitch_ws(ws: WebSocket, db: AsyncSession = Depends(get_db)):
    """FreeSWITCH WebSocket audio endpoint.

    Works with:
    - mod_twilio_stream (Twilio-like JSON frames)
    - mod_audio_stream community/commercial (metadata text + raw bytes, or JSON base64)
    """
