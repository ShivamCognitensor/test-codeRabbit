...
class BolnaWebhookPayload(BaseModel):
    ...
    def status_norm(self) -> str:
        ...
        """
        Normalize the Bolna-provided status by lowercasing and replacing hyphens with underscores.
        
        Returns:
            str: The normalized status string; empty string if the original status is missing or falsy.
        """
    ...
    def duration_seconds(self) -> Optional[int]:
        """
        Return the call duration in seconds extracted from the payload's telephony_data.
        
        Returns:
            int: Duration in seconds if `telephony_data` is a dict containing the "duration" key, `None` otherwise.
        """
    ...
    def recording_url(self) -> Optional[str]:
        """
        Get the recording URL from the payload's telephony_data.
        
        Returns:
            The recording URL string if available, `None` otherwise.
        """
        if isinstance(self.telephony_data, dict):
...
def verify_bolna_webhook(
    ...
    """
    Validate the incoming Bolna webhook using the configured shared secret.
    
    Parameters:
        x_bolna_webhook_secret (Optional[str]): Value of the `X-Bolna-Webhook-Secret` header sent by Bolna.
    
    Returns:
        True if verification passed or verification is disabled via configuration.
    
    Raises:
        HTTPException: with status 401 if a configured webhook secret exists and the provided header does not match it.
    """
...
def _calculate_qualification_score(extracted: Dict[str, Any]) -> int:
    """
    Compute a qualification score (0–100) from extracted call data.
    
    Scores are additive from these signals:
    - intent or loan_type → up to 30 points
    - loan amount present → 15 points
    - income present → 20 points
    - employment present → 20 points
    - preferred_callback_time present → 15 points
    
    Parameters:
        extracted (Dict[str, Any]): Parsed fields extracted from the call (e.g., intent/loan_intent, loan_type, loan_amount, income, employment_type, preferred_callback_time).
    
    Returns:
        int: Integer score between 0 and 100.
    """
...
async def _fallback_post_call_analytics(service: VoiceBotService, transcript: str) -> Dict[str, Any]:
    """
    Attempt a fallback post-call analytics pass using the provided service and transcript.
    
    If a transcript is present and the VoiceBotService implements `run_post_call_analytics`, invoke it and return its result; on any failure or when no transcript or method is available, return an empty dict.
    
    Parameters:
        transcript (str): Call transcript to analyze.
    
    Returns:
        Dict[str, Any]: Analytics results as a dictionary, or an empty dict when no analytics are produced.
    """
    if not transcript:
        return {}
...
async def bolna_webhook(
    ...
    """
    Handle a Bolna.ai webhook for a call execution and update the matching CampaignContact.
    
    Updates the contact's status, call outcome, duration, recording URL, transcript, and collected/ extracted data.
    On `completed` it computes or falls back to post-call analytics, calculates a qualification score, invokes the voice bot service to process the call result, and may trigger the next campaign step. For terminal non-completed statuses it processes the terminal result and may also trigger campaign progression. If the contact is already finalized the webhook commits metadata and safely triggers campaign progression for duplicate `completed` callbacks.
    
    Parameters:
        payload: BolnaWebhookPayload containing the execution identifier, normalized status, optional transcript,
            extracted_data, telephony data (duration, recording URL), and error message.
    
    Returns:
        A success response dict with at least `execution_id` and `status`; when a contact is matched the response
        also includes `contact_id` and the contact's updated status.
    """
    logger.info(
        "Bolna webhook: execution_id=%s status=%s norm=%s",
        payload.execution_id,
...
async def bolna_webhook_health():
    """
    Health check endpoint that reports the Bolna webhook service status.
    
    Returns:
        dict: Success response containing a message and data with {"status": "ok"}.
    """
...
async def call_callback(
    ...
    """
    Process a call result for a contact received from an external voice bot provider.
    
    Parameters:
        contact_id (UUID): Identifier of the contact whose call result is being reported.
        status (str): Final or intermediate call status to apply to the contact.
        call_duration (int, optional): Call duration in seconds, if available.
        responses (dict, optional): Collected question/answer pairs or structured responses from the call.
        qualification_score (int, optional): Calculated qualification score for the contact (0-100).
        collected_data (dict, optional): Additional extracted or merged data to store with the contact.
    
    Returns:
        dict: Success payload containing `contact_id` (string), `status` (string), and `lead_id` (string or `None`).
    """
    service = VoiceBotService(db)
