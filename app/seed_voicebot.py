...
def utcnow_naive() -> datetime:
    ...
    This returns a datetime with no timezone information (tzinfo is None), suitable for storing in TIMESTAMP WITHOUT TIME ZONE columns.
    
    Returns:
        datetime: Current UTC datetime without tzinfo.
    """
...
def make_execution_id(i: int) -> str:
    """
    Produce an execution identifier string for a seeded contact.
    
    Parameters:
        i (int): Numeric index used in the identifier; formatted as a three-digit, zero-padded value.
    
    Returns:
        exec_id (str): Identifier in the form "seed-exec-<index>-<8hex>" where <index> is the zero-padded three-digit value of `i` and <8hex> is an eight-character hexadecimal suffix.
    """
...
async def main():
    """
    Seed a VoiceBot campaign and its contacts into the database and generate per-contact post-call analytics.
    
    Creates a VoiceBotCampaign record (ensuring it has an ID), iterates CONTACTS to create CampaignContact records, analyzes each contact's transcript and embeds the analysis into the contact's collected_data along with UI flags, assigns a bolna_execution_id per contact, skips contacts that already exist for the campaign, commits the transaction, and prints a brief summary including the campaign ID, inserted/skipped counts, and execution ID mappings.
    """
