...
async def lifespan(app: FastAPI):
    """
    Manage the application's startup and shutdown tasks for the FastAPI app.
    
    On startup, attempts to create or verify database tables and starts the campaign runner. On shutdown, stops the campaign runner and logs shutdown progress. If an exception occurs during startup (for example while initializing the database), the error is logged.
     
    Parameters:
        app (FastAPI): The FastAPI application instance whose lifespan this context manager manages.
    """
    # Startup
