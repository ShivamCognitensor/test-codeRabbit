...
class ChatbotService:
    ...
    def __init__(self, db: AsyncSession):
        """
        Initialize the ChatbotService with a database session and prepare lazy-loaded client placeholders.
        
        Parameters:
            db (AsyncSession): Asynchronous database session used for persisting conversations and messages; stored on the instance as `self.db`.
        """
        self.db = db
    ...
    def lead_ops_client(self):
        """
        Lazily load and return the Lead Ops client instance.
        
        Returns:
            lead_ops_client: The Lead Ops client instance, cached on first access for subsequent calls.
        """
    ...
    async def _generate_ai_response(
        ...
        """
        Generate an assistant reply using OpenAI guided by conversation history and enriched context.
        
        Parameters:
            user_message (str): The incoming user message to respond to.
            history (List[Dict[str, str]]): Recent conversation messages as dicts with `role` and `content`, ordered oldest to newest.
            user_context (Optional[Dict[str, Any]]): Optional lead/BRE and user-related context used to enrich the system prompt.
            context_type (Optional[str]): Optional context label (e.g., "lender_selection") that tailors the generated response.
            is_public (bool): Whether the session is public; when true, context is generalized and personal data is avoided.
        
        Returns:
            assistant_response (str): The assistant's generated reply text.
            intent (str): The inferred intent label for the user's message.
        """
        # Build context block
    ...
    def _build_context_block(
        ...
        """
        Constructs a contextual block to be inserted into the system prompt to tailor assistant responses.
        
        When is_public is True, returns a short public-chat guidance block that instructs the assistant to keep replies general and encourage registration. For authenticated users, if user_context indicates leads or BRE recommendations, includes a lead summary (lead number, loan type, requested amount, status, stage) and up to five lender recommendations (lender, product, interest rate, EMI, amount, tenure, processing fee, match score). If context_type equals "lender_selection", appends a note about helping the user compare lenders and guiding selection via the dashboard.
        
        Parameters:
            user_context (Optional[Dict[str, Any]]): Optional user-specific data. Expected keys:
                - "has_leads" (bool): whether the user has lead data.
                - "active_lead" (dict): lead fields like "lead_number", "loan_type_code", "requested_amount", "status_code", "stage".
                - "recommendations" (List[dict]): BRE recommendations with keys like "lender_name", "product_name", "interest_rate", "emi", "amount", "tenure", "processing_fee", "score".
            context_type (Optional[str]): Context hint such as "lender_selection" to tailor the block for comparing lenders.
            is_public (bool): If True, returns guidance for an unauthenticated/public chat session.
        
        Returns:
            str: A concatenated context block suitable for inclusion in the system prompt; may be a public guidance block, lead/recommendation details, a lender-selection note, or "No specific user context available." when no context exists.
        """
        blocks = []
...
- Guide them to select via the dashboard""")
    ...
    def _generate_fallback_response(
        ...
        """
        Selects a rule-based assistant response and an inferred intent when the OpenAI service is unavailable.
        
        Parameters:
            user_message (str): The user's input message.
            context_type (Optional[str]): Optional context label guiding fallback behavior (e.g., "lender_selection", "lead").
            user_context (Optional[Dict[str, Any]]): Optional enriched context; if present, may include a "recommendations" list of lender recommendation dicts used to produce recommendation responses.
        
        Returns:
            Tuple[str, str]: A tuple of (intent, response_text). `intent` is a short identifier such as "greeting", "lender_help", "faq", "lead_assistance", or "unknown". `response_text` is the assistant's plain-text reply tailored to the detected intent or a generic help prompt.
        """
    ...
    def _detect_intent(self, user_message: str, response: str) -> str:
        """
        Classifies the user's message into a chatbot intent label.
        
        Parameters:
            user_message (str): The user's input text to analyze.
            response (str): The assistant's reply (provided for signature compatibility; not used by the detector).
        
        Returns:
            intent (str): One of the intent labels:
                'greeting', 'lender_selection', 'eligibility_check', 'document_help',
                'status_check', 'rate_inquiry', 'application_help', or 'general_query'.
        """
        message_lower = user_message.lower()
    ...
    async def _get_or_create_conversation(
        ...
        """
        Retrieve an active chat conversation for a session, creating and persisting a new one if none exists.
        
        Parameters:
            session_id (str): Unique identifier for the chat session.
            user_id (Optional[UUID]): User identifier associated with the conversation, if available.
            context_type (Optional[str]): Logical context label for the conversation (e.g., "lead", "lender_selection").
            context_id (Optional[UUID]): Identifier for the object referenced by context_type, if any.
            is_public (bool): Whether the conversation is a public/anonymous session.
        
        Returns:
            ChatConversation: The active conversation record for the given session (newly created and persisted if required).
        """
    ...
    async def _get_recent_messages(
        ...
        """
        Retrieve recent chat messages for a conversation ordered oldest to newest.
        
        Messages are limited to `limit` (defaults to settings.CHATBOT_MAX_HISTORY).
        
        Parameters:
            conversation_id (UUID): ID of the conversation to fetch messages for.
            limit (int, optional): Maximum number of messages to return; if omitted, uses settings.CHATBOT_MAX_HISTORY.
        
        Returns:
            List[Dict[str, str]]: A list of message objects with keys `role` and `content`, ordered from oldest to newest.
        """
    ...
    async def _get_message_count(self, conversation_id: UUID) -> int:
        """
        Return the total number of messages for the given conversation.
        
        Returns:
        	Total number of ChatMessage records associated with the conversation, or 0 if none exist.
        """
        result = await self.db.execute(
    ...
    async def get_conversation_history(
        ...
        """
        Retrieve the active chat conversation for a session.
        
        Parameters:
            session_id (str): Identifier for the chat session to look up.
            limit (int): Maximum number of recent messages to include in the returned conversation's history.
        
        Returns:
            ChatConversation | None: The active ChatConversation for the given session_id, or `None` if no active conversation exists.
        """
        query = select(ChatConversation).where(
            and_(
                ChatConversation.session_id == session_id,
    ...
    async def end_conversation(self, session_id: str) -> bool:
        """
        Mark the active chat conversation for the given session as closed.
        
        If an active conversation exists, sets its status to "CLOSED" and records the end timestamp.
        
        Returns:
            True if a conversation was found and closed, False otherwise.
        """
        query = select(ChatConversation).where(
            and_(
                ChatConversation.session_id == session_id,
    ...
    async def get_public_session_info(self, session_id: str) -> Dict[str, Any]:
        """
        Provide public chat session status and usage for UI rate limiting.
        
        Returns:
            info (dict): A dictionary with session usage and limit information:
                - exists (bool): `True` if a public conversation with the given session_id exists, `False` otherwise.
                - session_id (str, optional): The provided session_id when `exists` is `True`.
                - messages_used (int): Approximate number of user messages consumed in the session.
                - messages_remaining (int): Number of user messages left before the public chat limit is reached (min 0).
                - is_limit_reached (bool, optional): `True` when `messages_used` is greater than or equal to the configured public chat limit.
        """
        query = select(ChatConversation).where(
            and_(
                ChatConversation.session_id == session_id,
