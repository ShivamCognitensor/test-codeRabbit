...
class LocalRealtimeSession:
    ...
    def __init__(self) -> None:
        """
        Initialize a LocalRealtimeSession with settings-backed defaults and audio buffering.
        
        Creates a LocalSessionConfig populated from global settings (STT/TTs/LLM providers and default voicebot stack), prepares an empty PCM16 @ 16 kHz buffer for incoming audio, and initializes runtime control fields: last-audio timestamp, processing flag, and an asyncio Event used to trigger forced responses.
        """
        self.s = get_settings()
    ...
    def update_from_session(self, session: Dict[str, Any]) -> None:
        """
        Update the session configuration from a session dictionary.
        
        Reads well-known keys from `session` and updates the corresponding fields on
        the instance's LocalSessionConfig. Missing or falsy values in `session` leave
        existing configuration unchanged.
        
        Parameters:
        	session (Dict[str, Any]): Session payload that may contain:
        		- "instructions": assistant system prompt
        		- "input_audio_format": input audio format identifier
        		- "output_audio_format": output audio format identifier
        		- "voice": preferred voice identifier
        		- "input_audio_transcription": optional dict with "language" key
        		- "metadata": optional dict with non-standard overrides:
        			- "stt_provider", "tts_provider", "llm_provider", "voicebot_stack_id"
        """
    ...
    def append_audio_pcm16_16k(self, pcm16_16k: bytes) -> None:
        """
        Append raw PCM16 audio (16 kHz) bytes to the session's internal buffer and update the last-audio timestamp.
        
        Parameters:
            pcm16_16k (bytes): Raw little-endian PCM16 audio sampled at 16,000 Hz. If empty, the call is a no-op.
        """
        if not pcm16_16k:
            return
    ...
    def force_response(self) -> None:
        """
        Signal the session to emit a response on the next loop iteration.
        
        This notifies the session's background loop to treat buffered audio as ready for processing immediately; it does not block.
        """
        self._force_response.set()

    def take_buffer(self) -> PCM16Audio:
        """
        Return the session's accumulated PCM16 audio as a PCM16Audio object and clear the internal buffer.
        
        Returns:
            PCM16Audio: PCM16 audio containing the buffered bytes with a 16000 Hz sample rate.
        """
    ...
    async def _llm_reply(self, user_text: str) -> str:
        """
        Generate an assistant reply using the session's configured LLM provider.
        
        This will use the session configuration to choose between an OpenAI-compatible chat completions endpoint or a Voicebot LLM endpoint and produce a textual reply. Supported provider modes include "openai_compat" (OpenAI-style /chat/completions) and "voicebot" (FinAI Voicebot service).
        
        Returns:
            A string containing the assistant's reply. Returns an empty string when `user_text` is empty. On configuration or runtime failures, returns a short apology/error message string.
        """
        if not user_text:
            return ""
    ...
    async def process_turn(self) -> tuple[str, str, bytes]:
        """
        Run a single interaction: transcribe buffered audio, generate an LLM reply, and synthesize response audio.
        
        If no audio is buffered, returns empty user and assistant texts and empty bytes.
        
        Returns:
            tuple[str, str, bytes]: (user_text, assistant_text, audio_bytes)
            - user_text: transcribed user utterance, or "" if none.
            - assistant_text: generated assistant reply, or "" if none.
            - audio_bytes: synthesized audio for the assistant reply, or b"" if none.
        """
    ...
    async def run_loop(self, send_json) -> None:
        """
        Run a continuous background loop that detects end-of-turn conditions and emits response events.
        
        This coroutine monitors the session's incoming PCM buffer and timing to decide when to process a turn. It triggers a response when a forced-response event is set (and the buffered audio meets the minimum duration) or when recent audio has stopped for the configured silence window and the buffered audio meets the minimum duration. When a turn is triggered, it calls the internal response emitter to produce transcription, assistant text, and audio deltas. The loop respects the session's processing flag to avoid concurrent turns and is intended to run until cancelled.
        
        Parameters:
            send_json (Callable[[dict], None]): Callback used to send JSON events to the client.
        """
        silence_s = 0.65
        min_audio_ms = 450
...
async def _emit_response(sess: LocalRealtimeSession, send_json) -> None:
    """
    Process a single conversational turn and emit transcription, transcript, and audio delta events to the client.
    
    This calls the session to perform STT -> LLM -> TTS, then sends zero or more JSON messages via `send_json`:
    - If speech was transcribed, sends a "conversation.item.input_audio_transcription.completed" event with the transcribed text.
    - If an assistant reply was produced, sends a "response.output_audio_transcript.done" event with the transcript.
    - Streams the synthesized audio as successive "response.output_audio.delta" events containing base64-encoded 40 ms audio chunks.
    
    Parameters:
        sess (LocalRealtimeSession): Session instance used to process the turn.
        send_json (Callable[[dict], Awaitable[None]]): Async callable that sends a JSON-serializable dictionary to the client.
    """
