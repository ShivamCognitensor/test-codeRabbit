...
async def extract_post_call_analytics(
    ...
    """Best-effort extraction. Supports:
    - provider=openai_compat: OpenAI-compatible endpoint (OPENAI_BASE_URL + OPENAI_API_KEY)
    - provider=openai: OpenAI official (same client, also works with base_url)
    - provider=disabled: returns empty dict
    """
    s = get_settings()
