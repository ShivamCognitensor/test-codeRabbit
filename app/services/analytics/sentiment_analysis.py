...
def _get_sentiment_pipeline():
    """
    Lazily load and return a cached HuggingFace sentiment-analysis pipeline.
    
    Returns:
        The initialized HuggingFace `pipeline` instance configured for sentiment analysis (DistilBERT fine-tuned on SST-2, truncated to 512 tokens and returning all label scores).
    """
...
def _get_sentence_model():
    """
    Lazily load and cache a SentenceTransformer model for encoding sentence embeddings.
    
    Returns:
        SentenceTransformer: the cached SentenceTransformer instance (model "sentence-transformers/all-MiniLM-L6-v2") loaded on first call.
    """
...
def _contains_any(text: str, phrases: list[str]) -> bool:
    """
    Check whether any of the given phrase substrings appear in the text, using a case-insensitive substring match.
    
    Parameters:
        text (str): Input text to search.
        phrases (list[str]): Iterable of phrase substrings to look for; each phrase is matched as a substring (case-insensitive).
    
    Returns:
        bool: `True` if any phrase is found in `text`, `False` otherwise.
    """
    lowered = text.lower()
...
def _extract_callback_time(text: str) -> Optional[str]:
    """
    Extracts a preferred callback time expression from the transcript text.
    
    Searches for common phrasing such as "tomorrow at 3pm", "at 4pm tomorrow", or "tomorrow evening" and returns the first matched substring. If no detailed time is found but the word "tomorrow" appears, returns the string "tomorrow". Returns None when no relevant time expression is present.
    
    Returns:
        Matched time phrase (e.g., "tomorrow at 3pm" or "tomorrow"), or None if no time expression is found.
    """
    patterns: list[str] = [
...
def _compute_satisfaction(
    ...
    """
    Compute a heuristic satisfaction score (0–100) from keyword hits, a cooperation flag, and a transformer-derived positive score.
    
    Parameters:
        positive_hits (int): Count of detected positive keywords/phrases in the transcript.
        negative_hits (int): Count of detected negative keywords/phrases in the transcript.
        cooperative (bool): Whether the customer appeared cooperative/polite.
        hf_positive_score (float): Transformer model's positive-class confidence in the range 0.0–1.0.
    
    Returns:
        int: Satisfaction score between 0 and 100, where higher values indicate greater inferred customer satisfaction. The result is clamped to the 0–100 range.
    """
    base = 50
...
def _classify_sentiment(text: str) -> tuple[str, float]:
    """
    Classify sentiment from the transcript's customer utterances and return a mapped label with the model's positive score.
    
    If customer-labelled turns ("Customer:") exist, only those are analyzed; otherwise the full transcript is used. The function maps SST-2 POSITIVE/NEGATIVE scores into one of: `positive`, `neutral_positive`, `neutral`, `neutral_negative`, or `negative`.
    
    Returns:
        label (str): One of `positive`, `neutral_positive`, `neutral`, `neutral_negative`, `negative`.
        positive_score (float): The SST-2 `POSITIVE` score in the range 0.0–1.0.
    """
...
def _semantic_callback_check(text: str) -> tuple[bool, float]:
    """
    Determine whether the customer's utterances semantically express a callback request and return the highest positive-intent similarity.
    
    Parameters:
        text (str): Transcript text containing speaker-prefixed lines (e.g., "Customer: ...").
    
    Returns:
        tuple[bool, float]: `(is_callback, max_positive_similarity)` where `is_callback` is `True` if the maximum cosine similarity between any customer utterance and reference callback intents meets the module's callback threshold and exceeds the best negative-intent similarity by more than 0.05; `max_positive_similarity` is the maximum cosine similarity to positive (callback) intents, between 0.0 and 1.0.
    """
...
def _decide_callback(
    ...
    """
    Decide whether a callback should be requested based on semantic, keyword, and override signals.
    
    Parameters:
        semantic_callback (bool): True if semantic intent detection indicates a callback request.
        keyword_callback (bool): True if keyword matching indicates a callback request.
        negative_override (bool): True if a negative override (explicit opt-out) is present.
    
    Returns:
        bool: `True` if a callback should be requested, `False` otherwise.
    
    Behavior:
        - If `negative_override` is True, always returns False.
        - Otherwise returns True if either `semantic_callback` or `keyword_callback` is True.
    """
...
def analyze_transcript(transcript: str) -> AnalysisResult:
    """
    Analyze a call transcript and produce a structured post-call analysis.
    
    Performs sentiment classification, semantic and keyword-based callback intent detection, objection and human-handoff detection, computes satisfaction and confidence scores, and generates a short textual summary.
    
    Parameters:
        transcript (str): Full call transcript (agent and customer turns) to analyze.
    
    Returns:
        AnalysisResult: Aggregated analysis including `sentiment`, `satisfaction_score` (0–100), `interest_level` ("high", "medium", "low", or "none"), `callback_requested` (bool), `human_handoff_recommended` (bool), optional `preferred_callback_time`, optional `objection_type` (e.g., "wrong_number", "busy", "not_interested"), `customer_cooperative` (bool), `confidence` (0.0–1.0), and a human-readable `summary`.
    """
